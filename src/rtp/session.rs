...
pub async fn rtp_session_handler(
    ...
    info!("🎧 RTP OTURUMU BAŞLADI (Modüler v3).");
        ...
        tokio::select! {
            ...
            Some(command) = command_rx.recv() => {
                ...
                if handlers::handle_command(
                    command, config.port, &live_stream_sender, &permanent_recording_session, 
                    &mut outbound_stream_rx, &mut is_streaming_active, &mut playback_queue, 
                    &mut is_playing_file, &config, &socket, &playback_finished_tx,
            ...
            Some(_) = playback_finished_rx.recv() => {
                ...
                if let Some(next_job) = playback_queue.pop_front() {
                    ...
                    handlers::start_playback(next_job, &config, socket.clone(), playback_finished_tx.clone()).await;
                }
            },
