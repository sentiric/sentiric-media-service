...
async fn main() -> Result<()> {
    ...
    let s3_key = format!("{}/realistic_flow_{}.wav", s3_key_prefix, rtp_port);
    ...
    // DÜZELTME: user_sim_handle değişkeni _ ile başlatıldı (unused variable uyarısı için)
    let _user_sim_handle = tokio::spawn(
    ...
    assert!(stt_total_bytes > 115_200, "HATA: Gelen ses verisi az.");
    ...
    assert!(duration > 4.0, "HATA: Kayıt süresi çok kısa!");
...
async fn download_from_s3(client: &S3Client, bucket: &str, key: &str) -> Result<Vec<u8>> {
    ...
    for attempt in 1..=MAX_RETRIES {
        match client.get_object().bucket(bucket).key(key).send().await {
            ...
            Err(e) => {
                if attempt == MAX_RETRIES { return Err(anyhow::anyhow!(e)); }
